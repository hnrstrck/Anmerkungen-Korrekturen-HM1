...
{\small Es können keine weiteren Punkte für die Studienleistung erreicht werden. Deshalb hier nur einige Anregungen zu den Blättern, die mir bei der Durchsicht der Aufgaben gekommen sind.}
\end{center}
\end{mdframed}

\begin{itemize}
\item Denkt an die Gleichung für die Vielfachheiten für Eigenwerte. Die hilft manchmal.

\item Interessant sind solche Aufgaben, wo man z.\,B. nur Spur, Determinante und einen Eigenwert einer Matrix kennt und die restlichen bestimmen soll.

Dabei gilt:

Ist $A \in \R^{n \times n}$ und $z$ ein Eigenwert (also eine Nullstelle des charakteristischen Polynoms), dann ist $\overline{z}$ auch automatisch eine Nullstelle und damit ein Eigenwert. \textbf{Aber Achtung!} Für diesen Schluss braucht man zwingend, dass $A \in \R^{n \times n}$ ist. Wäre $A \in \C^{n \times n}$, wäre der Schluss nicht ohne weiteres richtig. 

Das liegt daran, dass für eine reelle Matrix (also $A \in \R^{n \times n}$) das charakteristische Polynom auch nur reelle Koeffizienten hat (also ein Polynom in $\R$ ist). Eine komplexe Matrix ($A \in \C^{n \times n}$) muss nicht unbedingt ein charakteristisches Polynom nur in $\R$ haben, da können echt reelle und echt komplexe Koeffizienten gemischt auftreten. Und dann ist $\overline{z}$ neben $z$ nicht unbedingt Nullstelle.

\item \textbf{Durch Matrizen kann man nicht teilen!}

Möchte man die Gleichung
\begin{align*}
AB = C
\end{align*}
nach $A$ auflösen, rechnet man \textbf{nicht} 
\begin{align*}
A = \frac{C}{B},
\end{align*}
sondern multipliziert mit der Inversen von $B$, sprich $B^{-1}$ (mal vorausgesetzt, dass diese existiere). Dabei muss die Richtung angegeben werden, da die Matrixmultiplikation in der Regel nicht kommutativ ist, d.\,h. es gilt in der Regel
\begin{align*}
AB \neq BA.
\end{align*}
Richtig wäre also:
\begin{align*}
AB = C \qquad \Leftrightarrow \qquad A = CB^{-1}
\end{align*}

\item Guckt euch die Sätze zu Eigenwerten und -vektoren von transponierten und inversen Matrizen an. Da gibt es einige spannende Zusammenhänge.

\item Mit am interessantesten ist die Aufgabe 1) f) (?) auf dem Blatt. Weil die Musterlösung da etwas knapp ausfällt, hier einige Worte dazu.

Wenn nur bekannt ist, dass für jeden Eigenwert die algebraische gleich der geometrischen Vielfachheit ist, heißt das, dass es eine \emph{Basis aus Eigenvektoren} gibt.

Warum? Es kann für eine Matrix $A \in \K^{n \times n}$ natürlich maximal $n$ (nicht notwendigerweise verschiedene) Eigenwerte geben (Begründung über den Grad des charakteristischen Polynoms). Ferner werden für die Basis linear unabhängige Vektoren des $\K^n$ gesucht, von denen es maximal $n$ Stück gibt. Da die Summe aller algebraischen Vielfachheiten gleich $n$ ist (das gilt immer!) und hier $\mu_a(\lambda_i ) = \mu_g(\lambda_i)$ für alle Eigenwerte gilt, ist also auch die Summe aller geometrischen Vielfachheiten gleich $n$ (diese Aussage ist natürlich nicht allgemein richtig, sondern nur hier unter diesen Besonderen Voraussetzungen). 

Da die Dimension eines Vektorraums die Länge einer Basis dieses Raums beschreibt, findet man also hier für jeden Eigenraum eine Basis aus so vielen Eigenvektoren, wie durch die geometrische Vielfachheit (bzw. die algebraische) angegeben wird (die geometrische Vielfachheit ist ja Dimension des Eigenraums). Da ferner Eigenvektoren zu verschiedenen Eigenwerten linear unabhängig sind (das gilt immer!), kommen sich die Eigenvektoren der verschiedenen Eigenräume nicht in die Quere, sodass es tatsächlich insgesamt $n$ linear unabhängige Eigenvektoren gibt, die eine Basis bilden. 

Was ist nun mit der \emph{Ortho-Normal-Basis aus Eigenvektoren}? Die gibt es allgemein nicht. Ich mache das graphisch klar. 

Sei $B \in \R^{2 \times 2}$ mit zwei Eigenwerten $\lambda_1$ und $\lambda_2$. Dann gilt nach Voraussetzung 
\begin{align*}
\mu_a(\lambda_1) = 1 = \mu_g(\lambda_1)
\end{align*}
und
\begin{align*}
\mu_a(\lambda_2) = 1 = \mu_g(\lambda_2).
\end{align*}

Das heißt, dass die beiden Eigenräume Geraden im $\R^2$ sind, die zum Beispiel so liegen könnten:
...
\begin{tikzpicture}[domain=-4:4] 
    ...
    \draw[<->] (-4.2,0) -- (4.2,0);
    \draw[<->] (0,-4.2) -- (0,4.2); 
    \draw[color=OliveGreen,domain=-3.9:3.9,smooth,variable=\x]   	plot (\x,{-0.2*\x})             node[right] {$E(\lambda_1)$}; 
    \draw[color=blue,domain=-3.9:3.9,smooth,variable=\x]    		plot (\x,{0.4*\x})             	node[right] {$E(\lambda_2)$}; 
\end{tikzpicture}
}
\end{figure}

Natürlich lässt sich in diesem Falle eine Basis aus Eigenvektoren angeben (siehe oben~-- zwei Eigenvektoren genügen, um den gesamten Raum $\R^2$ aufzuspannen). 

Da die Eigenräume aber nicht senkrecht aufeinander stehen (das ist durch das Bild offensichtlich), würde man mit \textsc{Gram-Schmidt} ansetzen, um eine ONB zu bestimmen. Dabei wird allerdings (hier) der Eigenraum $E(\lambda_2)$ verlassen (blau), sodass man zwar einen senkrechten (Basis-)Vektor $u$ (rot) zu einem Eigenvektor aus $v_1 \in E(\lambda_1)$ (grün) finden würde, aber $u$ wäre dann kein Eigenvektor mehr (nicht auf der grünen oder blauen Linie)! Es gilt -- wie im Bild gut zu erkennen -- offenbar: $u \notin E(\lambda_1)$ und $u \notin E(\lambda_2)$. Damit existiert keine \emph{ONB aus Eigenvektoren} (sondern nur eine \enquote{normale} ONB, von der \enquote{zufällig} ein Basisvektor auch ein Eigenvektor ist).
...
\begin{tikzpicture}[domain=-4:4] 
    ...
    \draw[color=OliveGreen,domain=-3.9:3.9,smooth,variable=\x]   	plot (\x,{-0.2*\x})             node[right] {$E(\lambda_1)$}; 
    \draw[color=blue,domain=-3.9:3.9,smooth,variable=\x]    			plot (\x,{0.4*\x})             	node[right] {$E(\lambda_2)$}; 
	\draw[color=red,domain=-1:1,smooth,variable=\x]    				plot (\x,{4*\x})             	node[right] {$\notin E(\lambda_1), \notin E(\lambda_2)$}; 
    \end{tikzpicture}
}
\end{figure}

Natürlich kann es vorkommen, dass für eine Matrix $A$ zufällig beide Eigenräume schon orthogonal sind. Dann existiert natürlich auch eine ONB aus Eigenvektoren, da dann auch bei \textsc{Gram-Schmidt} keine \enquote{neuen} Vektoren auftauchen / nichts mehr passiert.
\begin{figure}[H]
\centering
\scalebox{0.6}{
\begin{tikzpicture}[domain=-4:4] 
    ...
    \draw[<->] (-4.2,0) -- (4.2,0);
    \draw[<->] (0,-4.2) -- (0,4.2); 
    \draw[color=WildStrawberry,domain=-1.4:1.4,smooth,variable=\x]   	plot (\x,{3*\x})             node[right] {$E(\lambda_1)$}; 
    \draw[color=BurntOrange,domain=-3.9:3.9,smooth,variable=\x]    		plot (\x,{-0.3333*\x})             	node[right] {$E(\lambda_2)$}; 
\end{tikzpicture}
}
\end{figure}

Das ist mit dem \enquote{i.A. kann keine ONB [aus EV] gefunden werden} in der Musterlösung gemeint. Das soll an dieser Stelle ausreichen, ihr könnt gerne im Tutorium nachfragen.

\item Wenn eine Matrix die Eigenwerte $\lambda_1 = 1$, $\lambda_2 = 1$ und $\lambda_3 = 2$ hat, dann reicht es natürlich aus, nur für die Eigenwerte $\lambda_{1,2}$ und $\lambda_3$ die Eigenräume zu bestimmen (zwei Eigenraum-Bestimmungen).

\item Nicht so oft geübt, solltet ihr aber trotzdem können: \textbf{Eigenwerte sehen!} (TB 12, A2, c) –- Matrix $C \in \R^{5 \times 5}$). Für eine $5 \times 5$-Matrix möchte man nicht das charakteristische Polynom ausrechnen! Für die Rechnungen siehe Musterlösung. Die Idee ist, sich zu fragen, wie oft man auf der Hauptdiagonalen $1$ abziehen muss, um linear abhängige Zeilen zu erzeugen (sei das Ergebnis dieser Überlegungen / Berechnungen mit $D$ bezeichnet). Das \enquote{wie oft} ist der Eigenwert, und dass \enquote{auf der Hauptdiagonalen abziehen} das $- \lambda E$ in der Berechnung der Eigenraumberechnung (sprich $\text{kern}(A-\lambda E)$ wie man es kennt).

Es ist dann das LGS
\begin{align*}
Dx = (A - \lambda E)x = 0_{\R^n}
\end{align*}
zu lösen. Das Ergebnis ist der Eigenraum, der auch die Eigenvektoren liefert.

Ansätze mit der Spur von $A$ lassen dann meist auf die anderen Eigenwerte schließen.

\item LGS solltet ihr auch in den komplexen Zahlen lösen können. Das braucht ihr, wenn z.\,B. plötzlich komplexe Eigenwerte auftreten und ihr den Eigenraum bestimmen sollt.
\end{itemize}
...
\section*{Blatt 13}
\begin{itemize}
\item Alle Grenzwerte beziehen sich auf den Fall
\begin{align*}
...
\item Ihr müsst \enquote{alle $n$ gleichzeitig} gegen unendlich laufen lassen.

Das hier ist \textbf{falsch}:
\begin{align*}
\left(1 + \frac{1}{n}\right)^n \longrightarrow (1 + 0)^n = 1^n \longrightarrow 1.
\end{align*}

Es gilt ja bekanntermaßen
\begin{align*}
...
\end{align*}
verallgemmeinert werden.

Es gibt zwei Möglichkeiten, den Grenzwert von bspw.
\begin{align*}
\left(1 + \frac{3}{n}\right)^{4n}
\end{align*}
auszurechnen.

\begin{itemize}
\item \emph{Möglichkeit 1}

Mit den Potenzgesetzen gilt:

\begin{align*}
\left(1 + \frac{3}{n}\right)^{4n} = \left(\left(1 + \frac{3}{n}\right)^{n}\right)^4 \longrightarrow \left(e^{3}\right)^4 = e^{12}.
\end{align*}

\item \emph{Möglichkeit 2}

Es gilt
\begin{align*}
\left(1 + \frac{3}{n}\right)^{4n} = \left(1 + \frac{4\cdot 3}{4\cdot n}\right)^{4n} = \left(1 + \frac{12}{4n}\right)^{4n} \longrightarrow e^{12},
\end{align*}
da für eine konvergente Folge auch jede Teilfolge gegen denselben Grenzwert konvergiert. Das \enquote{$4n$} beschreibt diese Teilfolge.
\end{itemize}

\item Vorsicht! In der Regel gilt
\begin{align*}
0 \cdot \infty \neq 0.
\end{align*}
...
\begin{align*}
4\cdot\frac{1}{n}\cdot n \longrightarrow 4,
\end{align*}
obwohl 
\begin{align*}
...
\textbf{Gegenbeispiel.} Andererseits gilt
\begin{align*}
4\cdot\frac{1}{n^2}\cdot n \longrightarrow 0,
\end{align*}
\enquote{obwohl} 
\begin{align*}
...
n \longrightarrow \infty
\end{align*}
gilt. Man kann also keine generelle Aussage treffen.


Andere Fälle, die in dieselbe Richtung gehen, lauten
\begin{align*}
\infty - \infty \neq 0
\end{align*}
oder
...
Man überlegt sich leicht ähnliche Beispiele, die die Ungleichungen zeigen oder widerlegen.

\item Sofern bei Wurzeln Folgen im Radikaten stehen, ist der Grenzwert mit dem Einschachtelungssatz wie im Tutorium zu zeigen. Den Zusammenhang
\begin{align*}
\lim_{n \rightarrow \infty} \sqrt{a_n} = \sqrt{\lim_{n \rightarrow \infty} a_n}
\end{align*}
lernen wir erst später kennen (dieser darf nicht in der HM\RM{1}-Klausur benutzt werden). Dafür braucht man die Stetigkeit der Wurzelfunktion, die Stoff in der HM\RM{2} ist.

\item Beim Monotoniekriterium geht es nur darum, \emph{ob} eine Folge konvergent ist. Es wird keine Aussage darüber gemacht, was der Grenzwert ist.

Vergleichbar: \enquote{Ist die Matrix $A$ invertierbar?} im Gegensatz zu \enquote{Wie lautet die Inverse von $A$?}.

\item Immer dran denken, wenn ihr Kandidaten für Grenzwerte von (rekursiven) Folgen bestimmt: Eine Folge hat immer nur genau einen Grenzwert (sofern die Folge konvergent ist).

\item Folgen, die für $n \rightarrow \infty$ gegen $\pm \infty$ gehen, sind divergent. Die Aussage \enquote{Die Folge konvergiert gegen $\infty$} ist falsch, auch wenn man dies ab und an mal so sagt. Genaugenommen müsste es \enquote{Die Folge divergiert gegen $\infty$} heißen. 

\item Es gilt natürlich 
\begin{align*}
\pm\infty \notin \R, \C
\end{align*}
\end{itemize}
