...
\begin{itemize}
\item[a)] \textbf{Trick} 3. binomische Formel im Zähler. 

Und aufpassen: $\frac{1}{2}(1 + 3i)$ ist \textbf{nicht} in der Form $a+ ib$. Punktabzug (Aufgabenblatt!).

\item[b)] Auf den richtigen Winkel achten (Quadrant!). Es muss erkennbar sein, dass die Winkeltabelle benutzt wurde!

\enquote{Beide Winkel} müssen in der PKD gleich sein.

\item[c)] Skizze machen, wo die Zahl $z = 8$ liegt. Dann kann der Winkel abgelesen werden (Winkel $\alpha = \pi$ ist falsch!).

$\sqrt[3]{8} = 2$ darf ausgerechnet werden (sollte man wissen).
...
\item[e)] \textbf{Trick} Bei der i) muss ein $1^k$ ergänzt werden, um den binomischen Lehrsatz anwenden zu können. Dann erhält man $(1+i)^8$. Mit dem Hinweis, dass das Ergebnis reell sein soll, rechnet man wie folgt (schneller als \textsc{DeMoivre}). 
\begin{align*}
...
\textbf{Trick} Bei der ii) keine Indextransformation machen. Das macht den Binomialkoeffizienten unschön. Guckt man sich an, was $x$ und $y$ sind, so stellt man fest, dass $x + y = -2 + 2 = 0$ ist. Es ist also sinnvoller, den bin. Lehrsatz für $k=0$ bis $k = 11$ anzuwenden (es ergibt sich $0$) und dann den Summanden, den man zu viel gezählt hat (es ist der Summand für $k = 0$) vom Ergebnis abzuziehen. Man erhält dann $(-2)^{11} = -2048$.

\item[f)] Aufpassen! Wenn ihr vergesst, einen $x$-Wert zu überprüfen, gibt es Punktabzug. Es müssen minimal drei Fälle untersucht werden (Betrag führt zu zwei Fällen und die Ungleichung -- durch das $x$ im Nenner --  auch zu zweien. Diese können in drei Fällen zusammengefasst werden).

Definitionslücke beachten. Ein Fall wie $x < 1$ ist zu grob (was ist mit $x = -1$?).

Auch wichtig: Lösungsmengen für die einzelnen Fälle angeben! Und die Gesamtlösungsmenge $\Lsg_{\text{Ges}}$ angeben (richtig vereinigen -- sonst Punktabzug, s. Anmerkungen oben).
...
\begin{itemize}
\item[a)] Beim (IA) darf in der Summe nicht sofort das $k$ durch $2$ ersetzt sein (siehe Anmerkungen zum entsprechenden Übungsblatt). Punktabzug.
...
Rechenregeln der Fakultät beachten. 

\textbf{Trick} Die beiden Summen nach Einsetzten der (IV) nicht auf einen Hauptnenner bringen, sondern den Summanden, in dem $n!$ im Nenner steht, mit $(n+1)$ erweitern, sodass beide Nenner gleich sind ($n! \cdot (n+1) = (n+1)!$).

\item[b)] \textbf{Trick} $z\overline{z} = |z|^2$ und $-z+\overline{z} = -2\text{Im}(z)$. 

Zum Bestimmen von Real- und Imaginärteil der Lösung einen Koeffizientenvergleich machen.

Klar: Nicht die Lösung $a = -3$ vergessen bei $a = \sqrt{9}$. Sonst Punktabzug.
...
\textbf{Trick} Da $f$ nur Koeffizienten aus $\R$ hat, ist $n_2 := \overline{n_1} = -2i - 1$ auch eine Nullstelle. 

Dann kann man diese mit dem Horner Schema abspalten (geht wahrscheinlich schneller als die Division $f(z) : ((z - n_1)(z - n_2))$ und man erhält die dritte Nullstelle: $n_3 := -3$. 

Man kann dann die LFZ angeben. 
...
\item Gerade durch den Nullpunkt
\item Ebene durch den Nullpunkt
\item $\R^3$
\end{itemize}

Klar machen, warum das UVR von $\R^3$ sind. 

Weiterführende Fragestellung: Wie sehen alle Typen von UVR im $\R^2$ oder $\R^5$ aus?
\end{itemize}
...
\section*{Testat 3}
\begin{itemize}
\item Beim LGS kann man auch anders herum die richtige Lösung finden, indem man sich fragt, ob der Ortsvektor der Hyperebene der Lösungsmenge (die Lösungsmenge ist eine Hyperebene im $\R^6$) das LGS löst (Matrix-Vektor-Multiplikation).

Ansonsten die Variablen frei wählen, bei der keine Stufe in der ZSF ist. Das heißt, wenn zwei Variablen $x_k$ und $x_j$ frei gewählt werden können, wählt diese als frei, bei der der Index \textbf{größer} ist. Das haben wir bisher immer so gemacht (auch wenn das andere auch richtig ist). Siehe Tutorium.
